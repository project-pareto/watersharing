...
import pandas as pd
import json
from Utilities import fetch_case_data,fetch_match_data,fetch_distance_data,fetch_profit_data
...
from ipyleaflet import Map, basemaps, basemap_to_tiles, CircleMarker, Marker, MarkerCluster, AwesomeIcon, Popup, Polyline, LayerGroup, LegendControl
#import ipywidgets as widgets


##################################################
def SummaryPlot(input_dir,MATCHES,REQUESTS,DISTANCE,fig_name="plot.png",annotate_plot=True):
    """
    ...
    df_matches_supply, df_matches_demand, df_matches_producer_transport, df_matches_midstream_transport = fetch_match_data(MATCHES)
    ...
    ax = df_producer.plot.scatter(x = 'Longitude', y = 'Latitude', s = 100, c = "red", marker="^")
    if annotate_plot:
        for i in range(len(df_producer.index)):
            ...
            ax.annotate(df_producer["Supply Rate (bpd)"][i], (df_producer["Longitude"][i]+dx, df_producer["Latitude"][i]))

    # add consumer locations to plot
    df_consumer.plot.scatter(x = 'Longitude', y = 'Latitude', s = 100, color = "blue", marker="v", ax=ax)
    if annotate_plot:
        for i in range(len(df_consumer.index)):
            ...
            ax.annotate(df_consumer["Demand Rate (bpd)"][i], (df_consumer["Longitude"][i]+dx, df_consumer["Latitude"][i]))
    ...
    #date_nums_range = date2num(df_merge["Date"])
    ...
    df_merge['Date'] = df_merge['Date'].dt.strftime("%m/%d/%Y") # convert date to because DateTime isn't compatible with JSON
...
def SupplyPlotBar(input_dir,MATCHES,REQUESTS,ax):
    ...
    for ind,loc in df_matches_supply.iterrows():
        ...
        labels.append(loc["Supplier Index"]+" ("+loc["Date"].strftime('%m/%d/%Y')+")")

    # plot bars
    #plt.bar(x = x_coords, height = back_heights, color = "white", edgecolor = "red", label=labels)
    #plt.bar(x = x_coords, height = fore_heights, color = "red", edgecolor = "red")#, label=labels)
    ax.bar(labels, height = back_heights, color = "white", edgecolor = "red")
    ax.bar(labels, height = fore_heights, color = "red", edgecolor = "red")
    #ax = df_producer.plot.bar(x = "Wellpad", y = "Rate", color = "white", edgecolor="red")
    #df_matches_supply.plot.bar(x = "Wellpad", y = "Rate", color = "red", edgecolor="red", ax=ax)
...
def SupplyPlotBarByOperator(input_dir,MATCHES,REQUESTS,ax):
    ...
    # create lookup dictionary
    df_producer["Operator-Wellpad"] = df_producer["Operator"] + "-" + df_producer["Wellpad"]
    df_matches_supply["Operator-Wellpad"] = df_matches_supply["Operator"] + "-" + df_matches_supply["Supplier Wellpad"]
    BaseRates = pd.pivot_table(df_producer, values="Supply Rate (bpd)", index="Operator-Wellpad", aggfunc="sum")
    AllocationRates = pd.pivot_table(df_matches_supply, values="Rate", index="Operator-Wellpad", aggfunc="sum")
    ...
    for ind,loc in AllocationRates.iterrows():
        ...
        #back_heights.append(loc["Supply Rate (bpd)"]) # the next line is equivalent; shows how to index properly
        back_heights.append(BaseRates.loc[ind]["Supply Rate (bpd)"])
        fore_heights.append(AllocationRates.loc[ind]["Rate"])
        labels.append(ind)
        #labels.append(ind +" ("+loc["Date"].strftime('%m/%d/%Y')+")")
...
def DemandPlotBar(input_dir,MATCHES,REQUESTS,ax):
    ...
    for ind,loc in df_matches_demand.iterrows():
        ...
        fore_heights.append(loc["Rate"])
        labels.append(loc["Consumer Index"]+" ("+loc["Date"].strftime('%m/%d/%Y')+")")
...
def DemandPlotBarByOperator(input_dir,MATCHES,REQUESTS,ax):
    ...
    # create lookup dictionary
    df_consumer["Operator-Wellpad"] = df_consumer["Operator"] + "-" + df_consumer["Wellpad"]
    df_matches_demand["Operator-Wellpad"] = df_matches_demand["Operator"] + "-" + df_matches_demand["Consumer Wellpad"]
    BaseRates = pd.pivot_table(df_consumer, values="Demand Rate (bpd)", index="Operator-Wellpad", aggfunc="sum")
    AllocationRates = pd.pivot_table(df_matches_demand, values="Rate", index="Operator-Wellpad", aggfunc="sum")
...
def MidstreamPlotBar(input_dir,MATCHES,REQUESTS,ax):
    ...
    fore_heights = []
    labels = []
    pivot = pd.pivot_table(df_matches_midstream_transport[["Midstream Index","Rate"]], 
...
def MidstreamPlotBarByOperator(input_dir,MATCHES,REQUESTS,ax):
    ...
    # create lookup dictionary
    BaseRates = dict(zip(df_midstream["Operator"], df_midstream["Total Capacity (bbl)"]))
    ...
    labels = []
    try:
        pivot = pd.pivot_table(df_matches_midstream_transport[["Operator","Rate"]], 
                        index="Operator",
                        values="Rate",
                        aggfunc = "sum")
        pivot.reset_index(inplace=True)
    except:
        pivot = pd.DataFrame({"Operator":key,
                              "Rate":0}
                              for key in df_midstream["Operator"])
...
##################################################
def ProducerTransportPlot(input_dir,MATCHES,REQUESTS,fig_name="plot_producer_transport.png"):
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
...
def ProducerTransportPlotBar(input_dir,MATCHES,REQUESTS,ax):
    ...
    labels = []
    try:
        pivot = pd.pivot_table(df_matches_producer_transport[["Carrier Index","Rate"]], 
                            index="Operator",
                            values="Rate",
                            aggfunc = "sum")
...
def ProducerTransportPlotBarByOperator(input_dir,MATCHES,REQUESTS,ax):
    ...
    try:
        ...
        pivot = pd.DataFrame({"Carrier Index":key,
                              "Rate":0}
                              for key in df_producer["Index"])
...
##################################################
def AllocationPlotByOperator(input_dir,MATCHES,REQUESTS,fig_name="Allocations_AllInOne"):
    """
    Plots allocations on a series of subplots
    """
...
def InteractiveMapPlot(input_dir,MATCHES,REQUESTS):
    ...
    all_coords = pd.concat([df_producer[["Longitude","Latitude"]], df_consumer[["Longitude","Latitude"]]], ignore_index=True)
    ...
    m = Map(
        ...
        zoom=11, max_zoom = 14, min_zoom = 7, scroll_wheel_zoom = True,
    ...
    for ind,loc in df_matches_midstream_transport.iterrows():
        polyline = Polyline(
            locations = [(loc["From Latitude"],loc["From Longitude"]),(loc["To Latitude"],loc["To Longitude"])],
            color = "Gold",
            weight = 4,
            text=loc["Operator"],
            textposition="center"
            )
        polyline.popup = HTML(loc["Operator"]+": "+loc["From wellpad"]+" to "+loc["To wellpad"]+" ("+str(loc["Rate"])+")")
    ...
    for ind,loc in df_producer.iterrows():
        ...
        m.add_layer(popup)
        m.add_layer(Marker(icon=prod_icon,
            mode="markers+text",
            location=[loc["Latitude"],loc["Longitude"]],
            title = loc["Wellpad"],
    ...
    for ind,loc in df_consumer.iterrows():
        ...
        m.add_layer(popup)
        m.add_layer(Marker(icon=cons_icon,
            mode="markers+text",
            location=[loc["Latitude"],loc["Longitude"]],
            title = loc["Wellpad"],
            text=loc["Operator"]+"-"+loc["Wellpad"],
    ...
    legend = LegendControl({"Supplier":"red", "Consumer":"blue", "Midstream":"gold", "Supplier-Transport":"orange"}, title="Legend", position="bottomright")
    m.add(legend)
...
def NodalPricePlot(input_dir,MATCHES,REQUESTS,fig_name="plot_nodal_price"):
    ...
    all_coords = pd.concat([df_matches_supply[["Longitude","Latitude"]], df_matches_demand[["Longitude","Latitude"]]], ignore_index=True)
    ...
    obs_prices_df = pd.concat([df_matches_supply[["Nodal Price"]], df_matches_demand[["Nodal Price"]]], ignore_index=True)
    obs_prices = obs_prices_df.to_numpy()

    # generate RBF interpolation of observed data
    try:
        interp_flat = RBFInterpolator(obs_coords, obs_prices,kernel="thin_plate_spline",epsilon=1.5,smoothing=0.0025)(coord_flat)
        interp_grid = interp_flat.reshape(50, 50) # NOTE: must be the same as the grid spacing used for coord_grid
    except:
    ...
    fig.set_figwidth(15)
    ax.pcolormesh(*coord_grid, interp_grid, vmin=vmin, vmax=vmax, shading='gouraud', cmap=mpl.colormaps["Spectral"])
    p = ax.scatter(*obs_coords.T, c=obs_prices, s=50, ec='k', vmin=vmin, vmax=vmax, cmap=mpl.colormaps["Spectral"])
    cbar = fig.colorbar(p)
    #cbar.ax.set_ylabel("Price (USD/bbl)", rotation=270)
...
def ProfitPlot(input_dir,PROFITS,fig_name="plot_profit"):
    ...
    df_profits_supply, df_profits_demand, df_profits_producer_transport, df_profits_midstream_transport = fetch_profit_data(PROFITS)
    ...
    ax3 = plt.subplot(4,1,3)
    ax4 = plt.subplot(4,1,4)
    ax1.bar(x = df_profits_supply["Producer Index"].tolist(), height = df_profits_supply["Profit"].tolist(), color = "red")
    ax2.bar(x = df_profits_demand["Consumer Index"].tolist(), height = df_profits_demand["Profit"].tolist(), color = "blue")
    ax3.bar(x = df_profits_midstream_transport["Midstream Index"].tolist(), height = df_profits_midstream_transport["Profit"].tolist(), color = "gold")
    ax4.bar(x = df_profits_producer_transport["Producer Index"].tolist(), height = df_profits_producer_transport["Profit"].tolist(), color = "orange")
...
def ProfitPlotByOperator(input_dir,PROFITS,fig_name="plot_profit"):
    ...
    # pivot to get totals
    supply_pivot = pd.pivot_table(df_profits_supply[["Operator","Profit"]], index="Operator", values="Profit", aggfunc="sum")
    demand_pivot = pd.pivot_table(df_profits_demand[["Operator","Profit"]], index="Operator", values="Profit", aggfunc="sum")
    midstream_pivot = pd.pivot_table(df_profits_midstream_transport[["Operator","Profit"]], index="Operator", values="Profit", aggfunc="sum")
    p_transport_pivot = pd.pivot_table(df_profits_producer_transport[["Operator","Profit"]], index="Operator", values="Profit", aggfunc="sum")
    ...
    ax3 = plt.subplot(3,2,5)
    ax4 = plt.subplot(3,2,6)
    ax1.bar(x = [ind for ind,row in supply_pivot.iterrows()], height = supply_pivot["Profit"].to_list(), color = "red")
    ax1.set_ylim(GetLims(supply_pivot["Profit"].to_list()))
    ax1.tick_params(axis='x', labelrotation=45)
    ax2.bar(x = [ind for ind,row in demand_pivot.iterrows()], height = demand_pivot["Profit"].to_list(), color = "blue")
    ax2.set_ylim(GetLims(demand_pivot["Profit"].to_list()))
    ax2.tick_params(axis='x', labelrotation=45)
    ax3.bar(x = [ind for ind,row in midstream_pivot.iterrows()], height = midstream_pivot["Profit"].tolist(), color = "gold")
    ax3.set_ylim(GetLims(midstream_pivot["Profit"].to_list()))
    ax3.tick_params(axis='x', labelrotation=45)
    ax4.bar(x = [ind for ind,row in p_transport_pivot.iterrows()], height = p_transport_pivot["Profit"].tolist(), color = "orange")
...
def RevenuePlot(input_dir,MATCHES,fig_name="plot_revenue"):
    ...
    ax3 = plt.subplot(3,2,5)
    ax4 = plt.subplot(3,2,6)
    # NOTE: I am using the -1 in the below so that the resulting graph gives the expecgted visual interpretation of revenue, instead of the economic one
    ax1.bar(x = df_matches_supply["Supplier Index"].tolist(), height = (df_matches_supply["Nodal Price"]*df_matches_supply["Rate"]).to_list(), color = "red")
    ax2.bar(x = df_matches_demand["Consumer Index"].tolist(), height = (-1*df_matches_demand["Nodal Price"]*df_matches_demand["Rate"]).to_list(), color = "blue")
    if not df_matches_midstream_transport.empty:
        ax3.bar(x = df_matches_midstream_transport["Midstream Index"].tolist(), height = (df_matches_midstream_transport["Price"]*df_matches_midstream_transport["Rate"]).tolist(), color = "gold")
    if not df_matches_producer_transport.empty:
        ax4.bar(x = df_matches_producer_transport["Carrier Index"].tolist(), height = (df_matches_producer_transport["Price"]*df_matches_producer_transport["Rate"]).tolist(), color = "orange")
...
def RevenuePlotByOperator(input_dir,MATCHES,fig_name="plot_revenue"):
    ...
    df_matches_supply["Revenue"] = df_matches_supply["Nodal Price"]*df_matches_supply["Rate"]
    #df_matches_supply = df_matches_supply.fillna(0)
    supply_pivot = pd.pivot_table(df_matches_supply[["Operator","Revenue"]], index="Operator", values="Revenue", aggfunc="sum", fill_value=0, dropna=False)

    df_matches_demand["Revenue"] = df_matches_demand["Nodal Price"]*df_matches_demand["Rate"]*(-1) # NOTE: the negative one value here gives the expencted interpretation of revenue flow
    #df_matches_demand = df_matches_demand.fillna(0)
    demand_pivot = pd.pivot_table(df_matches_demand[["Operator","Revenue"]], index="Operator", values="Revenue", aggfunc="sum", fill_value=0, dropna=False)
    
    if not df_matches_midstream_transport.empty:
        df_matches_midstream_transport["Revenue"] = df_matches_midstream_transport["Price"]*df_matches_midstream_transport["Rate"]
        midstream_pivot = pd.pivot_table(df_matches_midstream_transport[["Operator","Revenue"]], index="Operator", values="Revenue", aggfunc="sum")

    if not df_matches_producer_transport.empty:
        df_matches_producer_transport["Revenue"] = df_matches_producer_transport["Price"]*df_matches_producer_transport["Rate"]
        p_transport_pivot = pd.pivot_table(df_matches_producer_transport[["Operator","Revenue"]], index="Operator", values="Revenue", aggfunc="sum")
    ...
    ax3 = plt.subplot(3,2,5)
    ax4 = plt.subplot(3,2,6)
    ax1.bar(x = [ind for ind,row in supply_pivot.iterrows()], height = supply_pivot["Revenue"].to_list(), color = "red")
    ax1.set_ylim(GetLims(supply_pivot["Revenue"].to_list()))
    ax1.tick_params(axis='x', labelrotation=45)
    ax2.bar(x = [ind for ind,row in demand_pivot.iterrows()], height = demand_pivot["Revenue"].to_list(), color = "blue")
    ax2.set_ylim(GetLims(demand_pivot["Revenue"].to_list()))
    ax2.tick_params(axis='x', labelrotation=45)
    if not df_matches_midstream_transport.empty:
        ax3.bar(x = [ind for ind,row in midstream_pivot.iterrows()], height = midstream_pivot["Revenue"].tolist(), color = "gold")
        ax3.set_ylim(GetLims(midstream_pivot["Revenue"].to_list()))
        ax3.tick_params(axis='x', labelrotation=45)
    if not df_matches_producer_transport.empty:
        ax4.bar(x = [ind for ind,row in p_transport_pivot.iterrows()], height = p_transport_pivot["Revenue"].tolist(), color = "orange")
