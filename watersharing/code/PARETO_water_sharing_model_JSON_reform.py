...
def create_model(restricted_set, df_producer, df_consumer, df_distance, df_time, objective_type, prev_obj, default={}):
    ...
    #first_date = min(pd.concat([df_producer['Start Date'].dt.strftime("%m/%d/%Y"),df_consumer['Start Date'].dt.strftime("%m/%d/%Y")], axis=0))
    #last_date = max(pd.concat([df_producer['Start Date'].dt.strftime("%m/%d/%Y"),df_consumer['Start Date'].dt.strftime("%m/%d/%Y")], axis=0))
...
def create_dataframe(model):
    ...
    df_v_Truck['Date']= df_v_Truck['Date'].dt.strftime("%m/%d/%Y")
    return df_v_Truck